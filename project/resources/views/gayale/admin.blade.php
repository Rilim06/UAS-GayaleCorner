...
<body>
    <nav class="navbar">
        <ul class="navbar-nav">
            ...
            <li class="nav-item">
                <a href="category/clothes" class="nav-link">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 650 650">
                        <g class="fa-group">
            ...
            <li class="nav-item">
                <a href="category/foods" class="nav-link">
            ...
            <li class="nav-item">
                <a href="category/beverages" class="nav-link">
            ...
            <li class="nav-item">
                <a href="category/others" class="nav-link">
                    <svg aria-hidden="true" focusable="false" data-prefix="fad" data-icon="space-shuttle" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 512" class="svg-inline--fa fa-space-shuttle fa-w-20 fa-5x">
                        <g class="fa-group">
    ...
    <div class='all' id='blur'>
        <div class="px-4 h-full">
            <div class="sm:w-[90%] ms-auto">
                <div class="flex justify-between items-center">
                    <a href="gayale/create" class="">
                        <button class="bg-[#08243A] hover:bg-[#0d3b5f] text-white text-center text-md sm:text-lg py-3 px-4 mt-4 rounded-full lg:text-sm lg:px-5 lg:py-3 xl:text-lg">
                            Tambah Item
                        </button>
                    </a>
                    <form method="post" action="{{route('logout')}}" class="pt-4">
                        @csrf
                        <button class="bg-[#F98538] hover:bg-[#ff9854] focus:bg-[#d16f2e] text-white text-center text-md sm:text-lg py-3 px-4 mt-4 rounded-full lg:text-sm lg:px-5 lg:py-3 xl:text-lg" type='submit'>Logout</button>
                    </form>
                </div>
                <div class="menu-container pb-24 lg:mt-4">
                    <div class="w-full grid grid-cols-1 gap-4 lg:grid-cols-2 2xl:grid-cols-3">
                        @foreach ($products as $product)
                        <div class="shadow-[0px_3px_10px_0px_rgba(0,0,0,.25)] rounded-2xl p-4 food-card grid grid-cols-9 gap-4 items-center sm:p-8 lg:p-4 2xl:p-6">
                            <img class="photo w-full aspect-square rounded-md col-span-3 object-cover lg:rounded-xl" src="{{asset('storage/' . $product->photo)}}" />
                            <div class="flex flex-col h-full justify-around my-auto col-span-4">
                                <h1 class="text-xl leading-none sm:text-2xl lg:text-xl xl:text-2xl">{{$product->name}}</h1>
                                <h2 class="text-slate-500 text-sm mt-2 leading-none sm:text-xl lg:text-sm lg:mt-0 xl:text-lg">Category: {{$product->category}}</h2>
                                <h2 class="font-bold text-lg sm:text-2xl lg:text-xl xl:text-xl">Rp. {{$product->price}}</h2>
                            </div>
                            <div class="flex flex-col gap-4 justify-center text-center col-span-2">
                                <a class="bg-[#F98538] hover:bg-[#ff9854] focus:bg-[#d16f2e] text-white text-md p-2 rounded-lg shadow-[0px_2px_8px_0px_rgba(0,0,0,.25)] flex justify-center mx-auto w-full sm:text-lg lg:p-1" href="/gayale/{{$product->id}}/edit">Edit</a>
                                <form class="bg-[#F98538] hover:bg-[#ff9854] focus:bg-[#d16f2e] text-white text-md p-2 rounded-lg shadow-[0px_2px_8px_0px_rgba(0,0,0,.25)] flex justify-center mx-auto mb-0 w-full sm:text-lg lg:p-1" action="/gayale/{{$product->id}}" method="post">
                                    @method('DELETE')
                                    @csrf
