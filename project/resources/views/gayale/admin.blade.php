    ...
    <div class='all' id='blur'>
        <div class="p-4 h-full">
            ...
            <div class="w-[90%] mx-auto">
                <a href="gayale/create" class=""><button class="bg-[#08243A] hover:bg-[#08243A] text-white text-center text-3xl py-4 px-8 mt-10 rounded-full lg:text-sm lg:px-5 lg:py-3">
                        Tambah Item
                    </button></a>
                <div class="menu-container mt-8 lg:mt-4">
                    <div class="w-full grid grid-cols-1 gap-8 lg:grid-cols-2 lg:gap-4">
                        @foreach ($products as $product)
                        <div class="shadow-[0px_3px_10px_0px_rgba(0,0,0,.25)] rounded-2xl px-12 py-8 food-card grid grid-cols-3 gap-12 lg:px-4 lg:py-4">
                            <img class="photo w-full aspect-square rounded-xl" src="{{asset('storage/' . $product->photo)}}" />
                            <div class="flex flex-col my-auto">
                                <h1 class="text-5xl lg:text-xl">{{$product->name}}</h1>
                                <h2 class="text-slate-500 text-sm mt-2 lg:mt-0">Category: {{$product->category}}</h2>
                                <h2 class="font-bold text-lg mt-6 lg:mt-1">Rp. {{$product->price}}</h2>
                            </div>
