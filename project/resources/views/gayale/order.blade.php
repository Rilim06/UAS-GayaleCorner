            ...
            <div class="w-[85%] mx-auto">
                ...
                <div class="menu-container mt-8 lg:mt-4">
                    <div class="w-full grid grid-cols-1 gap-8 lg:grid-cols-2 lg:gap-4 2xl:grid-cols-3">
                        ...
                        <div class="transactions shadow-[0px_3px_10px_0px_rgba(0,0,0,.25)] rounded-2xl px-12 py-8 food-card grid grid-cols-3 gap-12 lg:p-4 2xl:p-6 id=" {{ $status }}">
                            <div class="flex flex-col my-auto status-{{ $status }}" onclick='openPopup(
                                "{{ $transactionID }}",
                                "{{ $status }}",
                            ...
                            <div class="flex flex-col gap-4 justify-center text-center">
                                <button class="bg-[#F98538] hover:bg-[#ff9854] focus:bg-[#d16f2e] text-white text-2xl p-2 rounded-lg shadow-[0px_2px_8px_0px_rgba(0,0,0,.25)] flex justify-center mx-auto w-1/2 lg:w-full lg:text-lg lg:p-1" onclick='openBukti(@json($transactions->toArray()))'>Bukti Pembayaran</button>
                                <form action="/transaction/{{$transaction->transaction_id}}" method="post" id="updateForm{{$transaction->transaction_id}}">
                                    @method('PATCH')
                                    @csrf
                                    <select name="status" onchange="document.getElementById('updateForm{{$transaction->transaction_id}}').submit()">
                                        <option value="0" {{ $transaction->status == 0 ? 'selected' : '' }}>Pending
                                        </option>
                                        <option value="1" {{ $transaction->status == 1 ? 'selected' : '' }}>Packaging
                                        </option>
                                        <option value="2" {{ $transaction->status == 2 ? 'selected' : '' }}>Delivering
                                        </option>
                                        <option value="3" {{ $transaction->status == 3 ? 'selected' : '' }}>Arrived
                                        </option>
                                    </select>
                                </form>
                            </div>
